...
def populate(request):
    try:
        ...
        if table[0] not in formList:
            curr.execute(""" INSERT INTO ex04_movies(episode_nb, title, director, producer, release_date) VALUES
...
def remove(request):
    ...
    if (request.method == 'POST'):
        ...
        for k,v in name.items():
            if k == "sel":
                curr.execute("DELETE FROM ex04_movies WHERE title='"+v+"'")
                conn.commit()
    ...
    if (len(formList) == 0):
            return HttpResponse("No data available")
    return render(request, 'ex04.html', {"form":formList})